...
//
// Sample code that demonstrates the implementation of
// a 1D discrete Laplace operator in  both the "operator" form 
// and as a sparse matrix.
//
//
// Assuming the 270C_Support classes are contained in the directory
// ../270C_Support, the following commands can be used to compile
// this sample code.
//
// Debug mode with bounds checking:
//
// g++ -D_DEBUG LaplaceOp1d_Test.cpp -std=c++11 -I../270C_Support -o LaplaceOp1d_Test.exe
//
// Release mode with O2 optimization:
//
// g++ -O2 LaplaceOp1d_Test.cpp -std=c++11 -I../270C_Support -o LaplaceOp1d_Test.exe
//
// March, 28, 2019
...
#include "SparseOp/SCC_SparseOp.h"
...
public:
    ...
    Input  : SCC::DoubleVector1d a vector of size m whose values are those of
    ...
    on operations associated on the SCC::DoubleVector1d class, which
    is why there is no explicit bounds checking in this routine.
    */

    void apply(SCC::DoubleVector1d& V)
    {
    Vtmp = V;
    ...
    SCC::DoubleVector1d Vtmp; // Temporary
};
...
//   Problem set up. Unit domain.
    ...
    SCC::DoubleVector1d v(nx);
    SCC::DoubleVector1d vExact(nx);
    SCC::DoubleVector1d vErr(nx);
    ...
    SCC::SparseOp < SCC::DoubleVector1d > sOp(rowDimension, colDimension);
    //                     |
    //  Vector class with pre-defined set of member functions.
    //  See comments in SCC_SparseOp.h for required member functions.
    //
